...
//!
//! Spawns one PointLight per IFC light fixture, with color temperature and
//! intensity derived from the embedded EULUMDAT data. Lights are only spawned
//! when the user switches to Photometric or Combined mode (L key).
...
pub enum LightingMode {
    ...
    #[default]
    Architectural,
    /// IFC fixture lights only
    Photometric,
    /// Dimmed architectural + photometric
    Combined,
}
...
/// Marker for PointLight entities spawned by this module.
#[derive(Component)]
...
impl Plugin for PhotometricLightingPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<LightingMode>()
            ...
            .add_systems(Update, (poll_pending_lights, toggle_lighting_mode));
    }
}
...
fn poll_pending_lights(mut cache: ResMut<LdtCache>, mut state: ResMut<PhotometricState>) {
    ...
    crate::log_info(&format!(
        "[Photometric] Parsed {}/{} fixtures ({} unique LDT patterns). Press L to activate.",
        parsed,
        count,
        cache.parsed.len()
    ));
...
/// Toggle lighting mode with the L key.
fn toggle_lighting_mode(
    ...
    if !keys.just_pressed(KeyCode::KeyL) || state.fixtures.is_empty() {
        return;
    }
    ...
    *mode = mode.next();
    ...
    if needs_photometric && !state.lights_active {
        // Spawn one PointLight per fixture
        for fixture in &state.fixtures {
            commands.spawn((
                PointLight {
                    color: fixture.color,
                    intensity: fixture.lumens * 10.0,
                    radius: 0.05,
                    range: fixture.range,
