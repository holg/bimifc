...
pub mod shared {
    use super::*;
    use bimifc_bevy::{EntityInfo, IfcMesh, MeshGeometry};
    use bimifc_leptos::bridge::{EntityData, GeometryData};
    use std::sync::Arc;
...
pub fn main() {
    ...
    web_sys::console::log_1(&"[Unified] Starting BIMIFC Viewer (Leptos)".into());

    // Register the startBevyUnified function on window so Leptos can call it
    register_bevy_starter();

    // Initialize debug mode
    bimifc_leptos::bridge::init_debug_from_url();

    // Mount Leptos app
    leptos::mount::mount_to_body(bimifc_leptos::App);
}
...
fn register_bevy_starter() {
    ...
    let closure = Closure::wrap(Box::new(|canvas: String| {
        ...
        bimifc_bevy::run_on_canvas(&canvas);
...
pub fn start_bevy_unified(canvas_selector: &str) {
    ...
    bimifc_bevy::run_on_canvas(canvas_selector);
}
