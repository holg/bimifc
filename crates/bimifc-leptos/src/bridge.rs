    ...
    if let Some(window) = web_sys::window() {
        if let Ok(search) = window.location().search() {
            if search.contains("debug=1") || search.contains("debug=true") {
                ...
                // Always log this one
                web_sys::console::log_1(&"[BIMIFC] Debug mode enabled via URL".into());
            }
        }
    }
}

/// Storage keys (must match bimifc-bevy)
pub const GEOMETRY_KEY: &str = "bimifc_geometry";
...
pub fn save_geometry(geometry: Vec<GeometryData>) {
    ...
    #[cfg(feature = "unified")]
    {
        ...
        bimifc_bevy::set_pending_meshes(meshes);
